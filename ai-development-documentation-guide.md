...
#### スライス・シナリオ・テストをツリーで管理する

ここまで「1スライスにつき受け入れテスト1本」と説明してきたが、これは理解のための単純化である。実際には、**1つの業務シナリオを、役割の異なる複数のテストで検証する**ことがある。

管理上の階層は次のように考える。

| 階層 | 管理するもの | 例 |
|---|---|---|
| **機能・業務能力** | 利用者へ提供する大きな価値 | 商品価格を追跡する |
| **垂直スライス** | 独立して完成と判断できる増分 | 1商品を登録して現在価格を表示する |
| **受け入れシナリオ** | 利用者から見た具体的な振る舞い | 有効なURLを登録すると価格が表示される |
| **テスト群** | シナリオ内のリスクを検証する手段 | 受け入れ、ユニット、統合、契約、E2E |

```mermaid
flowchart TD
    F["機能：商品価格を追跡する"]

    F --> S1["PRICE-001<br/>1商品を登録して現在価格を表示する"]
    F --> S2["PRICE-002<br/>価格取得失敗を表示する"]
    F --> S3["PRICE-003<br/>複数商品を一覧表示する"]

    S1 --> R1["受け入れシナリオ<br/>有効なURLを登録すると価格が表示される"]
    R1 --> A1["受け入れテスト<br/>URL登録から価格表示まで"]
    R1 --> U1["ユニットテスト<br/>価格のパースと変換"]
    R1 --> I1["統合テスト<br/>DBへの保存と読み戻し"]
    R1 --> C1["契約テスト<br/>外部APIの形式"]
    R1 --> E1["代表的なE2Eテスト<br/>システム全体の接続"]
```

このツリーで重要なのは、**シナリオを技術層の途中で切らない**こと。`DBだけ完成` `APIだけ完成` `UIは未着手` では、利用者にとって何も完成していない。代わりに、シナリオの条件を狭くして端から端まで完成させる。

```
✗ 横方向の分割
  DBを作る → APIを作る → UIを作る → 最後に接続する
...
**「1スライス＝1つのメインシナリオ」**はよい出発点だが、厳密な1対1ではない。安全に完成と呼ぶために不可欠な異常系は、同じスライスへ含めてよい。判断基準は、**その範囲だけで安全に完成と呼べるか**である。

また、すべての条件をE2Eで検証する必要はない。シナリオは完結させたまま、シナリオ内のリスクごとに適したテストを選ぶ。

| 確認したいこと | 適したテスト |
|---|---|
| 利用者から見た代表的な振る舞い | 受け入れテスト |
| 計算、変換、分岐、境界値 | ユニットテスト |
| DBへの保存と読み戻し | 統合テスト |
| 外部APIやサービス間の形式 | 契約テスト |
| 画面からバックエンドまでの全体接続 | **少数の**E2Eテスト |

#### 状態は手作業で転記せず、GitとCIを正とする

ツリーは、スプレッドシートなどに人間が転記して維持する管理台帳ではない。**仕様、テストコード、Git、CIから辿れる構造**にする。実装者と管理者が異なるほど、手作業による二重管理は不整合を起こしやすい。

| 知りたい状態 | Source of Truth | 判定 |
|---|---|---|
| これから作るスライス | Issue / バックログ | Issueが存在する |
| 現在実装中のスライス | 作業ブランチ / PR | PRが開いている |
| 実装中のテスト結果 | ブランチ上のCI | 受け入れテストが赤→内側ループ中 |
| 完成したスライス | `main` の受け入れテスト | 必須テストがすべて緑 |
| 既存機能を壊したか | `main` / PRのCI | 過去に緑だったテストが赤 |
| なぜその仕様にしたか | 機能仕様 / ADR / コミット | Git履歴から参照する |

**運用フロー**

```mermaid
flowchart LR
    A["Issue<br/>未着手"] --> B["作業ブランチ・PR<br/>実装開始"]
    B --> C["受け入れテスト<br/>🔴 Red"]
    C --> D["内側のTDDループ"]
    D --> E{"必須テストが<br/>すべて緑か？"}
    E -- "いいえ" --> D
    E -- "はい" --> F["人間が受け入れ条件と差分をレビュー"]
    F --> G["mainへマージ<br/>🟢 完成済みシナリオ"]
    G --> H["CIが継続的に回帰を監視"]
```

`main` に将来分の赤いテストを大量に置くと、CIが常に赤になり、本当の回帰を判別できなくなる。未着手はIssue、赤い受け入れテストは作業ブランチ、緑になった完成済みテストだけを`main`へ着地させる。

**スライスのDefinition of Done**

- 受け入れ条件を人間が確定している
- スライスに必要な受け入れ・ユニット・統合・契約テストが緑
- 既存のテストを壊していない
- 必要な場合だけ、代表的なE2Eテストが緑
- 人間が受け入れ条件とコード差分をレビューしている
- `main`へマージされ、CIが緑を維持している

**作らないもの**

- テストごとの赤・緑を手入力するスプレッドシート
- コードやCIと重複する恒久的な実装状況一覧
- `main`を常時赤にする未実装シナリオのテスト
- テストコードから導出できる対応表の手書きコピー

規制対応などで要件とテストのトレーサビリティ表が必要な場合も、受け入れテストに`PRICE-001`のようなスライスIDを付け、**テストコードとCI結果から表を生成する**。人間が管理すべきなのは目的・範囲・期待値であり、実行結果と対応表はできる限り機械に導出させる。
